...
describe("authorization", () => {
  ...
  it("allows publish:own only for owned resources", () => {
    ...
    expect(canPublishArticle("user-1", ownPermissions, { ownerId: "user-1" })).toBe(true);
    expect(canPublishArticle("user-1", ownPermissions, { ownerId: "user-2" })).toBe(false);
  });
  ...
  it("supports can helper for article.publish action", () => {
    ...
    expect(can("user-1", ownPermissions, "article.publish", { ownerId: "user-1" })).toBe(true);
    expect(can("user-1", ownPermissions, "article.publish", { ownerId: "user-2" })).toBe(false);
    expect(can("user-1", anyPermissions, "article.publish", { ownerId: "user-2" })).toBe(true);
  });
});
